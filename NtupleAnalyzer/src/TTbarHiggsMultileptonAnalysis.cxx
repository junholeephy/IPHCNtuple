...
void TTbarHiggsMultileptonAnalysis::createHistograms()
{
   ...
   // Signal Region Two Leptons - Charge Flip
    ...
    theHistoManager->addHisto("MetLD_AN",                           "finalSel",     "WZ_CR",   "",  10,   0.,     2.);
    theHistoManager->addHisto("MinMll_AN",                          "finalSel",     "WZ_CR",   "",  10,   0.,   320.);
    theHistoManager->addHisto("MTW_AN",                             "finalSel",     "WZ_CR",   "",   9,   0.,   180.);
    ...
    theHistoManager->addHisto("MetLD_AN_Fakes",                           "finalSel",     "WZ_CR",   "",  10,   0.,     2.);
    theHistoManager->addHisto("MinMll_AN_Fakes",                          "finalSel",     "WZ_CR",   "",  10,   0.,   320.);
    theHistoManager->addHisto("MTW_AN_Fakes",                             "finalSel",     "WZ_CR",   "",   9,   0.,   180.);

    theHistoManager->addHisto("MTW_Fakes",                                "finalSel",     "WZ_CR",        "",  20,    0,   300);
...
void TTbarHiggsMultileptonAnalysis::Loop()
{
    ...
    for (Long64_t jentry=0; jentry<nentries_max;jentry++)
    {
        ...
        */
        ...
        if(DEBUG) std::cout << "New muons" << std::endl;
        ...
        if(DEBUG) std::cout << "New electrons" << std::endl;
        ...
        if(DEBUG) std::cout << "New taus" << std::endl;
        ...
        if( DEBUG ) std::cout << "Ordering" << std::endl;
        ...
        if ( vLeptons.size() >= 2) {
            for (unsigned int ilep = 0; ilep<vLeptons.size()-1; ilep++) {
                if ( vLeptons.at(ilep).pt() < vLeptons.at(ilep+1).pt() ) {
                    std::cout << "Run Event " << vEvent->at(0).run() <<  " " << vEvent->at(0).lumi() << " " << vEvent->at(0).id()
        ...
        if( DEBUG ) std::cout << "New jets / b-jets" << std::endl;
...
*/
            ...
            else if(vJet->at(ijet).CSVv2() < 0.5426)
            {
                ...
                if(vJet->at(ijet).eta() < 2.4)
                {
                    nForwardJets++;
                }
                else if(vJet->at(ijet).pt() > 40 && vJet->at(ijet).eta() > 2.4)
                {
                    nForwardJets++;
                }
            }
//---------
        ...
        if( DEBUG) std::cout << "New truth" << std::endl;
        ...
        if( DEBUG ) std::cout << "Going through selections" << std::endl;
        ...
        if ( print_all && ( is_3l_TTH_SR || is_3l_AppFakes_SR ) ) // || is_2lss_TTH_SR || is_2lss_AppFakes_SR ) )
        {
            ...
            std::cout << " Event:     run "     << vEvent->at(0).run()
                      << " lumi "               << vEvent->at(0).lumi()
                      << " event "              << vEvent->at(0).id()
        ...
        if( DEBUG) std::cout << "Madweight LHCO stuff" << std::endl;
        ...
        if( DEBUG) std::cout << "2D BDT stuff" << std::endl;
        ...
        if( DEBUG) std::cout << "Histogramz" << std::endl;
...
void TTbarHiggsMultileptonAnalysis::TwoLeptonsSameSignSelection_TTH2l(int evt)
{
    ...
    // ###############################################
    // #   ____  _____ ____ ___    ____  ____ _____  #
    // #  |  _ \| ____/ ___/ _ \  | __ )|  _ \_   _| #
    // #  | |_) |  _|| |  | | | | |  _ \| | | || |   #
    // #  |  _ <| |__| |__| |_| | | |_) | |_| || |   #
    // #  |_| \_\_____\____\___/  |____/|____/ |_|   #
    // #                                             #
    // ###############################################

    // variable implementation based on https://github.com/cms-ttH/ttH-13TeVMultiLeptons/blob/master/TemplateMakers/test/eventReconstructor_factorized.h#L48-L58


    // Fill variables

    // if (is2bTight)
    //     { signal_2lss_RecoBDT_MVA = mva_Reco_bTight->EvaluateMVA("BDTG method"); }
    // else
    //     { signal_2lss_RecoBDT_MVA = mva_Reco_bLoose->EvaluateMVA("BDTG method"); }
     ...
     // variable implementation based on https://twiki.cern.ch/twiki/bin/viewauth/CMS/TTHmultileptonsFor13TeV
...
void TTbarHiggsMultileptonAnalysis::ThreeLeptonSelection_TTH3l(int evt)
{
    ...
    if(DEBUG) std::cout << "Ok... SELECTED" << std::endl;
...
//NEW -------------------------------------------
void TTbarHiggsMultileptonAnalysis::ThreeLeptonSelection_THQ3l(int evt)
{
	v_cutflow[0]++;
    ...
    totCharge = vSelectedLeptons.at(0).charge() + vSelectedLeptons.at(1).charge() + vSelectedLeptons.at(2).charge();
    ...
    if(vSelectedLeptons.at(0).charge()!=totCharge) {dphiHighestPtSPPair = fabs(vSelectedLeptons.at(1).phi() - vSelectedLeptons.at(2).phi() );}
    else if(vSelectedLeptons.at(1).charge()!=totCharge) {dphiHighestPtSPPair = fabs(vSelectedLeptons.at(0).phi() - vSelectedLeptons.at(2).phi() );}
    else {dphiHighestPtSPPair = fabs(vSelectedLeptons.at(0).phi() - vSelectedLeptons.at(1).phi() );}
    ...
    if(do_printout)
    {
        ...
        cout<<"dphiHighestPtSPPair = "<<dphiHighestPtSPPair<<endl;
...
void TTbarHiggsMultileptonAnalysis::ThreeLeptonSelection_CR_WZ(int evt)
{
    ...
    for(int i=0; i<vLeptons.size()-1; i++)
    {
        for(int j=i+1; j<vLeptons.size(); j++)
        {
            if ( fabs( ( vLeptons.at(i).p4() + vLeptons.at(j).p4() ).M() ) < minMll )
            { minMll = fabs( ( vLeptons.at(i).p4() + vLeptons.at(j).p4() ).M() ) ;}
    ...
    theHistoManager->fillHisto("MetLD_AN",                       "finalSel", "WZ_CR",   "",   met_ld                , weight);
    theHistoManager->fillHisto("MinMll_AN",                      "finalSel", "WZ_CR",   "",   minMll                , weight);
    theHistoManager->fillHisto("MTW_AN",                         "finalSel", "WZ_CR",   "",   MTW                   , weight);

    theHistoManager->fillHisto("MTW",                            "finalSel", "WZ_CR",   "",   MTW                   , weight);
...
void TTbarHiggsMultileptonAnalysis::ThreeLeptonSelection_CR_WZ_ApplicationFakes(int evt)
{
    ...
    weight = weight * weightfake;

    // ##################################################################################################################################

    theHistoManager->fillHisto("MetLD_AN_Fakes",                       "finalSel", "WZ_CR",   "",   met_ld                , weight);
    theHistoManager->fillHisto("MinMll_AN_Fakes",                      "finalSel", "WZ_CR",   "",   minMll                , weight);
    theHistoManager->fillHisto("MTW_AN_Fakes",                         "finalSel", "WZ_CR",   "",   MTW                   , weight);

    theHistoManager->fillHisto("MTW_Fakes",                            "finalSel", "WZ_CR",   "",   MTW                   , weight);
...
void TTbarHiggsMultileptonAnalysis::initializeOutputTree()
{
    ...
    tOutput->Branch("is_3l_THQ_SR",&is_3l_THQ_SR,"is_3l_THQ_SR/B"); //NEW
    ...
    tOutput->Branch("nJet25",&nJet25,"nJet25/D");
    tOutput->Branch("MaxEtaJet25",&MaxEtaJet25,"MaxEtaJet25/D");
    tOutput->Branch("totCharge",&totCharge,"totCharge/D");
    tOutput->Branch("nJetEta1",&nJetEta1,"nJetEta1/D");
    tOutput->Branch("detaFwdJetBJet",&detaFwdJetBJet,"detaFwdJetBJet/D");
    tOutput->Branch("detaFwdJet2BJet",&detaFwdJet2BJet,"detaFwdJet2BJet/D");
    tOutput->Branch("detaFwdJetClosestLep",&detaFwdJetClosestLep,"detaFwdJetClosestLep/D");
    tOutput->Branch("dphiHighestPtSPPair",&dphiHighestPtSPPair,"dphiHighestPtSPPair/D");
    tOutput->Branch("minDRll",&minDRll,"minDRll/D");
    tOutput->Branch("Lep3Pt",&Lep3Pt,"Lep3Pt/D");
...
void TTbarHiggsMultileptonAnalysis::fillOutputTree()
{
    ...
    else if ( ((is_3l_THQ_SR || is_3l_TTH_SR || is_3l_TTZ_CR || is_3l_WZrel_CR) && vSelectedLeptons.size()==3)
            || ((is_3l_THQ_SR || is_3l_TTH_SR || is_3l_TTZ_CR || is_3l_WZrel_CR) && vSelectedLeptons.size()>=4) )
        is3l = true; //CHANGED
    ...
    if (vSelectedJets.size()<2) return; //-- CHANGED for tHq
    ...
    if( DEBUG ) std::cout << "Matching b-jets to gen ok" << std::endl;
    ...
    //std::cout << "Choosing 2 more jets ok" << std::endl;
    ...
    if( DEBUG ) std::cout << "Standalone Gen ok" << std::endl;
    ...
    */
    ...
    is_2bTight_float        = (vSelectedMediumBTagJets.size()>=2)?1.0:0.0;

    //std::cout << "End input to MEM ==========" << std::endl;
